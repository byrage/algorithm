    ...
    public static void main(String[] args) {
        ...
        for (int i = 0; i < computerCnt; i++) {
            for (int[] connection : connections) {
                if (infection.contains(connection[0]) || infection.contains(connection[1])) {
                    infection.add(connection[0]); infection.add(connection[1]);
                }
            }
        }

        System.out.println(infection.size() - 1);
    }
}
